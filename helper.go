...
func isIncluded(data codekit.M, f *dbflex.Filter) (bool, error) {
	...
	if f.Op == dbflex.OpEq {
		...
		*/
		...
		match := false
		for _, keyword := range keywords {
			if strings.EqualFold(strings.ToLower(dataValue), strings.ToLower(keyword)) {
				match = true
				break
			}
		}
...
func getValueOf(from reflect.Value, name string) interface{} {
	if from.Type().Kind() == reflect.Map {
		...
		fieldCount := from.NumField()
		for i := 0; i < fieldCount; i++ {
			fieldName := from.Type().Field(i).Name
			if strings.EqualFold(strings.ToLower(fieldName), strings.ToLower(name)) {
				return from.FieldByName(fieldName).Interface()
			}
		}
	}
	return nil
